...
const ProductListItems = ({product}) => {
    const {price,category, description, subs, shipping, color, brand, quantity, sold} = product;
    return (
        <ul className="list-group">
            ...
            {subs && (<li className="list-group-item">
                Sub Categories:  
                 {subs.map((s) => (<Link to ={`/sub/${s.slug}`} className="label label-default label-pill pull-xs-right">{s.name}</Link>))}
            ...
            <li className="list-group-item">
                Available Quantity: <span className="label label-default label-pill pull-xs-right">{quantity}</span>
            </li>
            <li className="list-group-item">
